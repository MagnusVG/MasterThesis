...
def main():
    ...
    for num_points in N_POINTS:
        # Read training data
        read_folder = "grids/points_"+str(num_points)+"/random/"
        ...
        # Get class weights
        class_weights = [10, 0.526] # 5% 1000/(2*50)
        ...
        for epoch in range(EPOCHS):
            ...
            print("Validation loss: {:.4f}".format(total_validation_loss / len(val_dataloader)))

        #classifier.save("tuning/batch_size/"+str(batch_size)+"/pn_model_dict"+args.save+".pt")
        np.save("tuning/npoints/"+str(num_points)+"/train_loss_single.npy", np.array(train_loss))
        np.save("tuning/npoints/"+str(num_points)+"/validation_loss_single.npy", np.array(validation_loss))
