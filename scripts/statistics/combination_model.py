...
def main():
    ...
    validation["quality"] = (validation.quality - np.min(validation.quality)) / (np.max(validation.quality) - np.min(validation.quality))

    train_data = np.asarray(data[["x", "y", "z", "quality"]])
    train_labels = np.asarray(data["accepted"])
    validation_data = np.asarray(validation[["x", "y", "z", "quality"]])
    ...
    conv_str = ""
    print("Normal model")
    input_shape = (np.sum([8*i for i in range(args.grid_size+1)])*4) + 4 + 2
